...
class Art
  ...
  def enhance
    ...
    (0...num_of_positions).each do |pos|
  ...
  def enhance_block(block)
    block_orientations = orientations(block)
    correct_pos = block_orientations.find do |position|
      rules[stringify(position)].present?
    end
    # byebug
    arrayify(rules[stringify(correct_pos)])
  end
...
def run1
  # art = Art.new(read_file('2017/p21_test.txt'))
  art = Art.new(read_file('2017/p21_input.txt'))
  5.times{art.enhance}
  p art.num_on

  13.times{art.enhance}
  p art.num_on
end
