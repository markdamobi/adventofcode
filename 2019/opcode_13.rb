...
class Program
  def initialize(list:, orig_input: [], pointer: 0, relative_base: 0, arcade: )
    @list = list
    @pointer = pointer
    @orig_input = orig_input
    @relative_base = relative_base
  ...
  attr_reader :list, :pointer, :orig_input, :relative_base, :last_code, :arcade
  ...
  def compute(input: nil)
    ...
    while(new_code != 99)
      if new_code == 3
        @orig_input = [0] if arcade.joystick_neutral?
        @orig_input = [-1] if arcade.joystick_left?
        @orig_input = [1] if arcade.joystick_right?
      end
      x = tick
      break if new_code == 4
      new_code = get_code
