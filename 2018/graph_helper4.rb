...
class Graph4 
  ...
  attr_reader :source, :vertices, :edges, :ss, :tt, :d, :g_edges, :targets, :targetds
  ...
  def dijkstra
    ...
    loop do 
      ...
      if target_min.present? && min_d > target_min
        return 
      end
      ...
      ## we've gotten minimums if this happens. no need to go further. 
      # byebug
      # # if targetds.size > 1
      # if target_min.present? && min_d > target_min
      #   return 
